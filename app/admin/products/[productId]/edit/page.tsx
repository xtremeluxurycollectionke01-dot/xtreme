...
export default function EditProductPage() {
  ...
  const addSize = () => {
    if (sizeInput && !formData.sizes.some(s => s.size === sizeInput)) {
      const system = sizeInput.split(' ')[0] || "US";
      setFormData(prev => ({
        ...prev,
        sizes: [...prev.sizes, { size: sizeInput, system, stock: 0 }]
      }));
      setSizeInput("");
    }
  };
  ...
  const addColor = () => {
    if (colorInput.trim() && !formData.colors.some(c => c.name === colorInput.trim())) {
      setFormData(prev => ({
        ...prev,
  ...
  const addImage = () => {
    if (imageUrl.trim() && !formData.images.some(img => img.url === imageUrl.trim())) {
      setFormData(prev => ({
        ...prev,
  ...
  const removeImage = (urlToRemove: string) => {
    setFormData(prev => {
      const filtered = prev.images.filter(img => img.url !== urlToRemove);
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  {formData.images.map((img, index) => (
                    <div key={img._id || index} className="relative group">
                      ...
                      <button
                        ...
                        className={`absolute bottom-2 left-2 px-2 py-1 text-xs font-bold rounded transition-colors ${
                          img.isPrimary 
            ...
            {/* Sizes */}
            <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
              <h2 className="text-xl font-bold mb-6">Available Sizes & Stock</h2>
                  ...
                  {formData.sizes.map((sizeObj, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-gray-500">({sizeObj.system})</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            type="number"
                            value={sizeObj.stock}
                            onChange={(e) => updateSizeStock(sizeObj.size, parseInt(e.target.value) || 0)}
                            min="0"
                            className="w-20 px-2 py-1 bg-gray-900 border border-gray-700 rounded text-center text-sm"
                          />
                        </div>
                        <button
                          type="button"
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.colors.map((colorObj, index) => (
                    <div
                      ...
                      className="inline-flex items-center gap-2 px-3 py-2 bg-gray-800 rounded-full"
                    >
                      <span 
                        className="w-4 h-4 rounded-full border border-gray-600"
                        style={{ backgroundColor: colorObj.hex }}
                      />
                      <span className="text-sm">{colorObj.name}</span>
                      <button
                        type="button"
                        onClick={() => removeColor(colorObj.name)}
